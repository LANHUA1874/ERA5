...
sta = pd.read_csv("F:/2020tk分层/stationCN.csv",encoding = 'gb2312',dtype={"id": str})
stationlist = sta['id']
...
for station in stationlist:
    ...
    for date in datelist_s:
        try:
            ...
            mkdir('F:/2020tk分层/' + station)
            df.to_csv('F:/2020tk分层/' + station + '/' + station + '_' + date.strftime('%Y%m%d%H') + '.csv', index=False)
